...
1. **크레스트 변경** 탭으로 이동합니다.
2. 교체할 팀의 **크레스트 ID**를 입력합니다. `1234`, `d1234`, `l1234` 형식 모두 인식됩니다.
   - ID를 입력하면 해당 ID의 현재 크레스트(엠블럼)가 위쪽 미리보기에 자동으로 표시됩니다. 올바른 팀인지 확인한 뒤 진행하세요.
3. **이미지 파일 선택** 버튼으로 교체할 이미지를 고르거나 드래그 앤 드롭합니다.
   - 권장 이미지 크기는 **256 × 256 픽셀 정방형**입니다. 비정방형 이미지는 자동으로 중앙 크롭됩니다.
   - 교체할 이미지 파일명이 `1234`, `d1234`, `l1234` 형식이면 ID 입력란이 자동으로 채워집니다. FC온라인에서 내보낸 원본 파일을 그대로 사용할 수 있습니다.
